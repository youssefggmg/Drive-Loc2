...
<body class="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 min-h-screen flex items-center justify-center">
    <div class="max-w-lg w-full bg-white shadow-2xl rounded-lg p-8">
        ...
        <form action="../controllers/addReservation.php" method="POST" id="dynamicForm"
            ...
            <!-- Vehicle Field -->
            <div>
                <label for="vehiculID" class="block text-sm font-medium text-gray-700 mb-1 hidden">Vehicle</label>
                <input type="number" hidden name="vehiculID" value="<?php echo $_GET["VID"] ?>">
            </div>

            <!-- User Field -->
            <div>
            <label for="vehiculID" class="block text-sm font-medium text-gray-700 mb-1 hidden">Vehicle</label>
            <input type="number" hidden name="userID" value="<?php echo $_COOKIE["userID"] ?>">
            </div>
